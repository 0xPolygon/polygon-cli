# `polycli heimdall estimate stake-exit`
...
Mark validator exit (MsgValidatorExit, L1-mirroring).

```bash
polycli heimdall estimate stake-exit [flags]
```
...
```bash
      ...
      --deactivation-epoch uint   deactivation epoch
      ...
      --from-msg string           MsgValidatorExit.from address (default: signer)
