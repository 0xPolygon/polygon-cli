# `polycli heimdall state-sync is-old`
...
Check whether an L1 state-sync event was already replayed.

```bash
polycli heimdall state-sync is-old <TX_HASH> <LOG_INDEX> [flags]
```
