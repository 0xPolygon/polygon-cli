...
- **`SpendOrWaitAvailableBudget(context.Context, uint64) error`**: Attempts to spend gas; blocks if insufficient budget is available or until context is cancelled
...
4. Start Provider:
  -> Begins watching for new blocks (polls every 1 second)
  -> When first block header is fetched: adds initial gas budget based on wave's starting Y value
  -> Note: Vault remains at zero budget until first header is received (~1 second delay)
...
1. Gas Limit Decision:
  -> Loadtest calls gasVault.SpendOrWaitAvailableBudget(ctx, gasLimit)
  -> If vault has sufficient budget: deducts amount and returns nil
  -> If vault lacks budget: blocks until provider adds more gas
  -> If context is cancelled: returns ctx.Err() for graceful shutdown
...
- [plot Usage](../../cmd/plot/usage.md)
- [Wave visualization examples](../../cmd/plot/examples/)