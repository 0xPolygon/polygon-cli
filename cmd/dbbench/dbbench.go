...
func init() {
	f := DBBenchCmd.Flags()
	f.Uint64Var(&writeLimit, "write-limit", 1000000, "The number of entries to write in the db")
	f.Uint64Var(&readLimit, "read-limit", 10000000, "the number of reads will attempt to complete in a given test")
	f.Uint64Var(&overwriteCount, "overwrite-count", 5, "the number of times to overwrite the data")
	f.BoolVar(&sequentialReads, "sequential-reads", false, "if true we'll perform reads sequentially")
	f.BoolVar(&sequentialWrites, "sequential-writes", false, "if true we'll perform writes in somewhat sequential manner")
	f.Uint64Var(&keySize, "key-size", 32, "The byte length of the keys that we'll use")
	f.Uint8Var(&degreeOfParallelism, "degree-of-parallelism", 2, "The number of concurrent goroutines we'll use")
	f.StringVar(&rawSizeDistribution, "size-distribution", borDistribution, "the size distribution to use while testing")
	f.BoolVar(&nilReadOptions, "nil-read-opts", false, "if true we'll use nil read opt (this is what geth/bor does)")
	f.BoolVar(&dontFillCache, "dont-fill-read-cache", false, "if false, then random reads will be cached")
	f.BoolVar(&readStrict, "read-strict", false, "if true the rand reads will be made in strict mode")
	f.BoolVar(&noWriteMerge, "no-merge-write", false, "allows disabling write merge")
	...
	f.IntVar(&cacheSize, "cache-size", 512, "the number of megabytes to use as our internal cache size")
	f.IntVar(&openFilesCacheCapacity, "handles", 500, "defines the capacity of the open files caching. Use -1 for zero, this has same effect as specifying NoCacher to OpenFilesCacher.")
	f.BoolVar(&writeZero, "write-zero", false, "if true, we'll write 0s rather than random data")
	f.BoolVar(&readOnly, "read-only", false, "if true, we'll skip all the write operations and open the DB in read only mode")
	f.StringVar(&dbPath, "db-path", "_benchmark_db", "the path of the database that we'll use for testing")
	f.BoolVar(&fullScan, "full-scan-mode", false, "if true, the application will scan the full database as fast as possible and print a summary")
	f.StringVar(&dbMode, "db-mode", "leveldb", "The mode to use: leveldb or pebbledb")
