...
func initFlags() {
	...
	ltp.BlobFeeCap = LoadtestCmd.Flags().Uint64("blob-fee-cap", 100000, "blob fee cap, or maximum blob fee per chunk, in Gwei")
	ltp.SendingAccountsCount = LoadtestCmd.Flags().Uint64("sending-accounts-count", 0, "number of sending accounts to use. This is useful for avoiding pool account queue")
	...
	ltp.SendingAccountsFile = LoadtestCmd.Flags().String("sending-accounts-file", "", "file containing sending accounts private keys, one per line. This is useful for avoiding pool account queue but also to keep same sending accounts for different execution cycles")
	ltp.MaxBaseFeeWei = LoadtestCmd.Flags().Uint64("max-base-fee-wei", 0, "maximum base fee in wei. If base fee exceeds this value, sending tx will be paused and while paused, existing in-flight transactions continue to confirmation, but no additional SendTransaction calls occur. This is useful to avoid sending transactions when network is congested")
...
v3, uniswapv3 - Perform UniswapV3 swaps`)
	...
	ltp.LoadtestContractAddress = LoadtestCmd.Flags().String("loadtest-contract-address", "", "address of pre-deployed load test contract")
	ltp.ERC20Address = LoadtestCmd.Flags().String("erc20-address", "", "address of pre-deployed ERC20 contract")
	ltp.ERC721Address = LoadtestCmd.Flags().String("erc721-address", "", "address of pre-deployed ERC721 contract")
	ltp.RecallLength = LoadtestCmd.Flags().Uint64("recall-blocks", 50, "number of blocks that we'll attempt to fetch for recall")
	ltp.ContractAddress = LoadtestCmd.Flags().String("contract-address", "", "address of contract that will be used in --mode contract-call. This must be paired up with --mode contract-call and --calldata")
	ltp.ContractCallData = LoadtestCmd.Flags().String("calldata", "", "hex encoded calldata passed in. Format is function signature + arguments encoded together. This must be paired up with --mode contract-call and --contract-address")
