...
func newCountCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			body, status, err := rest.Get(cmd.Context(), "/milestones/count", nil)
			if err != nil {
				return err
			}
			...
			if opts.JSON {
				m, jerr := decodeJSONMap(body, "milestone count")
				if jerr != nil {
					return jerr
				}
			...
			if resp.Count == "" {
				return fmt.Errorf("milestone count response missing count (body=%q)", truncate(body, 256))
			}
