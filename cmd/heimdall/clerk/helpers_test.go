...
func testdataPath(t *testing.T, subdir, name string) string {
	...
	// cmd/heimdall/clerk/<thisFile> -> ../../../internal/heimdall/client/testdata/<subdir>
...
type restRoute struct {
	status int
	body   []byte
	// match allows a route to inspect query parameters (e.g. to
	// disambiguate /clerk/event-records/list?page=1 from ?page=2).
	// When nil the route matches all queries.
...
// runCmd assembles a fresh root command with the state-sync umbrella
