...
func testdataPath(t *testing.T, subdir, name string) string {
	...
	// cmd/heimdall/span/<thisFile> -> ../../../internal/heimdall/client/testdata/<subdir>
...
// httptestServer spins up a test server whose response is chosen by
// matcher(path). matcher returns (body, status, matched). When
// matched==false, the server returns HTTP 404 with the raw body (or an
// error string when body is nil).
func httptestServer(t *testing.T, matcher func(path string) ([]byte, int, bool)) *httptest.Server {
	t.Helper()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, status, ok := matcher(r.URL.Path)
...
// runCmd assembles a fresh root command with the span umbrella wired
// in, using the given REST URL, and executes argv. Each call creates
// new cobra command instances so tests don't share state.
