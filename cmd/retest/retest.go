...
// WrapPredeployedCode will wrap a predeployed contract so that it can be deployed for testing. For now we're just wrapping
...
func init() {
	f := RetestCmd.Flags()
	f.StringVar(&inputFileName, "file", "", "Provide a file that's filed with test transaction fillers")
